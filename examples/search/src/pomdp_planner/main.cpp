...
public:
  ...
  World* InitializeWorld(std::string& world_type, DSPOMDP* model, option::Option* options)
  {
      ...
      //SearchWorld* world = new SearchWorld();
       //return InitializePOMDPWorld(world_type,model, options);
      SearchWorld* world = new SearchWorld(model, Seeds::Next());
      //Establish connection with external system
      world->Connect();
      //Initialize the state of the external system
      world->Initialize();
      //static_cast<Tiger*>(model)->NoiseSigma(SearchWorld::noise_sigma_);
